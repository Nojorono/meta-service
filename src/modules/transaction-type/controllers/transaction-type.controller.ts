...
export class TransactionTypeController {
  ...
  @ApiQuery({ name: 'transactionTypeName', required: false, description: 'Filter by transaction type name' })
  @ApiQuery({ name: 'transactionTypeDms', required: false, description: 'Filter by transaction type DMS' })
  ...
  async findAll(@Query() query: TransactionTypeQueryDto): Promise<any> {
    const data = await this.transactionTypeService.findAllTransactionTypes(query);
    const total = await this.transactionTypeService.countTransactionTypes(query);
    return {
      success: true,
      statusCode: 200,
  ...
  async findByName(@Param('name') name: string): Promise<any> {
    const data = await this.transactionTypeService.findAllTransactionTypes({ transactionTypeName: name });
    return {
      success: true,
      statusCode: 200,
      message: 'Transaction types retrieved successfully',
  ...
  async findByIdMicroservice(@Payload() id: number): Promise<TransactionTypeDto> {
    return await this.transactionTypeService.findTransactionTypeById(id);
  }

  @MessagePattern('transaction-type.findByName')
  async findByNameMicroservice(@Payload() name: string): Promise<TransactionTypeDto[]> {
    return await this.transactionTypeService.findAllTransactionTypes({ transactionTypeName: name });
  }
}
