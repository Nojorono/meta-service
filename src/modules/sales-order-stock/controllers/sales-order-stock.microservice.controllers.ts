...
export class SalesOrderStockMicroserviceController {
  constructor(private readonly salesOrderStockService: SalesOrderStockService) {}

  @Get()
  @ApiOperation({ summary: 'Get all sales order stock' })
  @ApiResponse({ status: 200, description: 'Sales order stock retrieved successfully', type: [SalesOrderStockDto] })
  ...
  @ApiQuery({ name: 'FLOW_STATUS_CODE', required: false, type: String, description: 'Flow status code' })
  @ApiQuery({ name: 'OPEN_FLAG', required: false, type: String, description: 'Open flag' })
