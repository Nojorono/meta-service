...
export class OrganizationService {
  ...
  async findAllOrganizations(
    ...
    try {
      const { organizationCode, organizationName, organizationType, locationCode, page = 1, limit = 10 } = queryDto;
  ...
  async findOrganizationById(organizationId: number): Promise<OrganizationDto | null> {
    try {
      const query = `
        SELECT 
          ORGANIZATION_ID,
      ...
      this.logger.error(`Error finding organization by ID ${organizationId}:`, error);
      throw error;
    }
  }

  async findOrganizationByCode(organizationCode: string): Promise<OrganizationDto | null> {
    try {
      const query = `
        SELECT 
          ORGANIZATION_ID,
      ...
      const result = await this.oracleService.executeQuery(query, [organizationCode]);
      
      if (result.rows.length === 0) {
        this.logger.warn(`Organization not found for code: ${organizationCode}`);
        return null;
      }
      ...
      this.logger.error(`Error finding organization by code ${organizationCode}:`, error);
      throw error;
    }
  }

  async getOrganizationCount(queryDto: OrganizationQueryDto = {}): Promise<number> {
    try {
      const { organizationCode, organizationName, organizationType, locationCode } = queryDto;
